...
def extract_api_calls(har_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for entry in har_data["log"]["entries"]:
        ...
        api_call = {
            ...
            "responseBody": response.get("content", {}).get("text", "")
...
def is_api_request(request: Dict[str, Any]) -> bool:
    ...
    if any(
        tracker in url for tracker in ["analytics", "tracking", "stats", "telemetry"]
    ):
        return False
...
def format_response_data(
    data: Any, max_lines: int = 10, max_chars_per_line: int = 80
) -> str:
    ...
    if isinstance(data, dict):
        ...
        if len(formatted_pairs) <= max_lines:
            ...
            return (
                ...
                + ",\n  ... ("
                + str(len(formatted_pairs) - max_lines)
                + " more keys)\n}"
            )
