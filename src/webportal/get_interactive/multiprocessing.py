...
def _worker(task_and_headless: tuple[IngestTask, bool]) -> str:
    ...
    result = ingest_from_prompt(task.prompt, unique_data_dir, headless)
    return result


def run_multiple_ingests(tasks: list[IngestTask], max_workers: int = None, headless: bool = True) -> list[str]:
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_index = {
            executor.submit(_worker, (tasks[i], headless)): i 
