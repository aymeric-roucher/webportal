...
def test_github_sponsor_batch_deferred():
    ...
    assert "error" in result or "data" in result or "item-" in str(result)
...
def test_github_label_validate():
    ...
    params = {"repo": "numpy/numpy", "q": "00 - Bug", "filter_value": "00 - Bug"}
...
def test_github_search_repositories():
    ...
    params = {"q": "numpy", "type": "repositories"}
...
def test_github_filtered_issues_page():
    ...
    url = "https://github.com/numpy/numpy/issues"
    params = {"q": 'is:issue state:open label:"00 - Bug" sort:created-asc'}
...
def test_invalid_repository_endpoint():
    ...
    assert (
        "error" in result or "404" in str(result) or "not found" in str(result).lower()
    )
...
def test_post_request_without_auth():
    ...
    assert (
        ...
        or "login" in str(result).lower()
        or "auth" in str(result).lower()
    )
